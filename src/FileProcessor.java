...
public class FileProcessor {
    public static void updateArrayList(ArrayList<Car> cars) throws FileNotFoundException {
        ...
        while (readCars.hasNextLine()) {
            ...
            if (readCars.equals("Family")) { // carInfo[0].equals("Family")
                //Car familyCar = new Family("Family", brand, model, fuelType, registration, odometer, manualGear, airCondition, cruiseControl, seat);
                Car familyCar = new Family("Family", carInfo[0], carInfo[1], carInfo[2], carInfo[3], Integer.parseInt(carInfo[4]), carInfo[5], carInfo[6], carInfo[7], Integer.parseInt(carInfo[8]));
                cars.add(familyCar);
                ...
                Car luxuryCar = new Luxury("Luxury", carInfo[0], carInfo[1], carInfo[2], carInfo[3], Integer.parseInt(carInfo[4]), Integer.parseInt(carInfo[5]), carInfo[6], carInfo[7], carInfo[8], carInfo[9]);
                cars.add(luxuryCar);
            } else if (readCars.equals("Sport")) {
                //Car sportCar = new Sport("Luxury", brand, model, fuelType, registration, odometer, manualGear, hp);
                Car sportCar = new Sport("Sport", carInfo[0], carInfo[1], carInfo[2], carInfo[3], Integer.parseInt(carInfo[4]), carInfo[5], Integer.parseInt(carInfo[6]));
                cars.add(sportCar);
            }
        }
        readCars.close();
